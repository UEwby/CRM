...
$(function () {
    ...
    Ajax({
        type: 'get',
        url: 'pc/weekly/getcrmWeeklyreporthezonginfo?weeklyid='+weeklyid
    }).then(function (res) {
        console.log(res);
        //顶部基本信息
        ...
        $.each(crmweeklyteambuildlist,function (i, item) {
            sum += parseFloat(NullZero(item.membercount));
            $('.content_table1 > tbody').append(
                '<tr>' +
                '<td>'+ item.deptmentname +'</td>' +
                '<td>'+ NullZero(item.membercount) +'</td>' +
                '<td>'+ NulltoK(item.worksummary) +'</td>' +
                '</tr>'
            )
        })
        $('.content_table1 > tbody').append(
